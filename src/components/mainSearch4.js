...
class mainSearch extends React.Component {
	...
	// clickの時、この行のカラーを変更する
/* 	handleChangeColor = () => {
		$('table tr').click(function () {
			if ($(this).find('td').hasClass('changeColor')) {
				$(this).find('td').removeClass('changeColor');
				$('button[name="clickButton"]').prop('disabled', true);
			}else {
				$(this).find('td').addClass('changeColor');
				$(this).siblings().find('td').removeClass('changeColor');
				...
				<Card className="border border-dark bg-dark text-white" style={{ "textAlign": "center" }}>
					...
					<Card.Body>
						<div style={{ "float": "right" }}  >
							<Button size="sm" variant="info" name="clickButton" onClick={this.employeeDetail}><FontAwesomeIcon icon={faList} />詳細</Button>{' '}
							<Button size="sm" variant="info" name="clickButton" onClick={this.employeeUpdate}><FontAwesomeIcon icon={faEdit} />修正</Button>{' '}
							<Button size="sm" variant="info" name="clickButton" onClick={this.employeeDelete}><FontAwesomeIcon icon={faTrash} />削除</Button>
						</div>
							...
							<tbody>
								{employeeList.length === 0 ?
									...
									currentEmployees.map(data =>
										<tr key={data.rowNo} className="background" name="background" onClick={this.handleChangeColor.bind(this, data.employeeNo)}>
											<td>{data.rowNo}</td>
